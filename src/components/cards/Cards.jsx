...
const Cards = ({ dataelement }) => {
  return (
    <Link href={`Collection/${dataelement.element.id}`}>
      <div className="flex-col relative" id={dataelement.element.id}>
        ...
        <Image
          alt={dataelement.element.name}
          src={dataelement.element.imgPath}
          width={392}
          height={438}
          className="card-image"
        ></Image>
        <h2 className="card-title">{dataelement.element.name}</h2>
        <div className="flex justify-between items-center">
          <p className="card-subtitle">{dataelement.element.description}</p>
          <p className="card-price">${dataelement.element.price}</p>
        </div>
      </div>
    </Link>
  );
