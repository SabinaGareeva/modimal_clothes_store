...
const Filter = (props) => {
  const { nameFilter,  options, checkedOption,handleCheckboxChange } = props;
  ...
  return (
    <div className={css.filter__container}>
      ...
      >
        ...
        >
          {`${nameFilter[0].toUpperCase()}${nameFilter.slice(1, nameFilter.length).toLowerCase()}`}
        </p>
        {filterIsOpen ? (
          <svg
      ...
      <div className={`p-[1.6rem] ${filterIsOpen ? "block" : "hidden"}`}>
        {options?.map((option) => (
          <div key={option}>
            ...
            >
              {`${option[0].toUpperCase()}${option.slice(1,option.length).toLowerCase()}`}
            </label>
          </div>
        ))}
      </div>
